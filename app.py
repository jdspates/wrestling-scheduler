...
# REAL EMOJIS — copy-paste these exact characters
EMOJI_MAP = {
    "red": "red circle",
    "blue": "blue circle",
    "green": "green circle",
...
for i in range(5):
    ...
    with col1:
        # SHOW REAL EMOJI (DOT ONLY)
        current_emoji = EMOJI_MAP[team["emoji"]]
        st.markdown(f"<div style='font-size:32px; text-align:center;'>{current_emoji}</div>", unsafe_allow_html=True)
...
# Session state
for key in ["bout_list","mat_schedules","suggestions","active","last_removed","initialized"]:
    if key not in st.session_state:
        st.session_state[key] = [] if key in ["bout_list","mat_schedules","suggestions","active"] else None if key=="last_removed" else False
...
if st.session_state.initialized:
    ...
    for mat_num in range(1, CONFIG["NUM_MATS"] + 1):
        ...
        mat_dfs[mat_num] = pd.DataFrame(rows)
