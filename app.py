...
if st.session_state.initialized:
    ...
    if search_term.strip():
        ...
        st.info(
            "Showing **" + str(len(filtered_active)) + "** wrestler(s) matching “" +
            search_term + "” (out of " + str(len(raw_active)) + " active)."
        )
    else:
        filtered_active = raw_active
        st.info("Showing **all " + str(len(filtered_active)) + "** active wrestlers.")
    ...
    if len(active_ids) < 2:
        ...
        if create_manual:
            if manual_w1_id == manual_w2_id:
                ...
                st.success(
                    "Manual match created: " + w1["name"] + " (" + w1["team"] + ") vs " +
                    w2["name"] + " (" + w2["team"] + "). You can now drag it to the desired mat and slot."
                )
                st.rerun()
    ...
    st.caption(
        "**" + str(under_count) + "** of **" + str(len(filtered_active)) +
        "** filtered wrestler(s) need more matches."
    )
    ...
    if current_suggestions:
        ...
        for i, s in enumerate(current_suggestions):
            ...
            sugg_data.append({
                "Add": False,
                "Current": str(len(w["match_ids"])),
                "Wrestler": f"{w['name']} ({w['team']})",
                "Lvl": f"{w['level']:.1f}",
                "Wt": f"{w['weight']:.0f}",
                "vs_Current": str(len(o["match_ids"])),
                "vs": f"{o['name']} ({o['team']})",
                "vs_Lvl": f"{o['level']:.1f}",
                "vs_Wt": f"{o['weight']:.0f}",
                "Score": f"{s['score']:.1f}",
    ...
    if visible_conflicts:
        st.warning(
            "Rest conflicts detected: **" + str(len(visible_conflicts)) +
            "** (requires at least **" + str(rest_gap) + "** matches between bouts for the same wrestler)."
        )
    else:
        st.caption("No rest conflicts found (min gap: " + str(rest_gap) + " matches).")
        ...
        if search_term.strip():
            ...
            for mat in range(1, CONFIG["NUM_MATS"] + 1):
                ...
                with st.expander(f"Mat {mat}", expanded=True):
                    ...
                    st.caption(
                        str(match_count) + " match" +
                        ("es" if match_count != 1 else "") +
                        " in this mat preview."
                    )
                    if match_count == 0:
                    ...
                    if mat_conflicts:
                        ...
                        for c in mat_conflicts:
                            st.markdown(
                                "- " + c["wrestler"] + " (" + c["team"] + "): " +
                                "Bout " + str(c["bout1"]) + " (Slot " + str(c["slot1"]) + ") → " +
                                "Bout " + str(c["bout2"]) + " (Slot " + str(c["slot2"]) + ") " +
                                "(gap " + str(c["gap"]) + " < required " + str(rest_gap) + ")"
                            )
            ...
            for mat in range(1, CONFIG["NUM_MATS"] + 1):
                ...
                with st.expander(f"Mat {mat}", expanded=True):
                    ...
                    for idx2, bn in enumerate(st.session_state.mat_order[mat], start=1):
                        ...
                        b = next(x for x in st.session_state.bout_list if x["bout_num"] == bn)
                        bout_label_map[bn] = (
                            "Slot " + str(idx2) + " – Bout " + str(bn) + ": " +
                            b["w1_name"] + " (" + b["w1_team"] + ") vs " +
                            b["w2_name"] + " (" + b["w2_team"] + ")"
                        )
